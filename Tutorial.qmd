...
There are many ways we can plot CGM data. The simplest of these is a time series graph. This is a line graph withn time on the X axis, and glucose value on the Y axis. It allows us to see all the data collected on a given participants, identify any periods of missing data, see the highest and lowest glucose values recorded, and do a basic quality check to identify any potential issues with our data. 

Another commonly plotted graph is an in range percentage plot. This graph tells us the percentage of time a participant spent below, above, and in range. 
...
For this exercise we would like you to plot three graphs: a time series graph for one participants data, an in range percentage plot for one participants data, and an AGP for one participants data. The iglu package has functions to create all these. Once you have created this series of graphs for one participant, please create the same series of graphs for another participant. Using these graphs try to compare the glucose control between the two. What is similar, what is different, does anything stand out?
...
data <- read.csv("data/Patient A.csv")
View(data)
```
...
iglu::glu(data |> dplyr::filter(id == "001"))
iglu::glu(data |> dplyr::filter(id == "002"))

iglu::plot_ranges(data |> dplyr::filter(id == "001"))
iglu::plot_ranges(data |> dplyr::filter(id == "002"))
